<script>
  import { 
    FolderOpen, Save, Plus, Trash2, ArrowUp, ArrowDown, X, FileUp, FileSpreadsheet, SaveAll,
  ...
  function onPointerUp(e) {
    ...
    if (isDragging && draggedIdx !== null && hoveredIdx !== null && draggedIdx !== hoveredIdx) {
  ...
  function parseStatus(text) {
    ...
    if (parts.length > 1) {
      ...
      if (mainUpper.includes("DANG XU LY CAU HINH")) {
        mainText = "Đang xử lý cấu hình " + mainUpper.match(/\((\d+\s*\/\s*\d+)\)/)?.[0] || "";
      } else if (mainUpper.includes("DANG PHAN TICH")) {
        mainText = "Đang phân tích cấu hình...";
      } else {
        mainText = mainText.charAt(0).toUpperCase() + mainText.slice(1).toLowerCase();
      }
...
{#if isDragging && ghostFile}
  ...
  >
    <div class="p-2 rounded-lg bg-zinc-500/5 shrink-0
    ...
    <div class="min-w-0 flex-1">
      ...
      <span class="px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider bg-zinc-500/10 text-[var(--text-muted)]">
...
<aside class="w-80 flex flex-col bg-[var(--sidebar-bg)] border-r border-[var(--border)] shrink-0 select-none z-10 shadow-sm">
  ...
  <div class="p-4 border-b border-[var(--border)] flex justify-between items-center shrink-0">
    <div class="flex flex-col gap-1 min-w-0">
      <span class="text-[9px] text-[var(--text-muted)] font-bold uppercase tracking-wider">Không gian làm việc</span>
      <span class="text-xs font-bold text-[var(--text)] truncate">
        {appState.currentProjectPath ? appState.currentProjectPath.split(/[\\/]/).pop() : "Chưa lưu cấu hình"}
      </span>
    </div>
  ...
  <div class="flex-grow overflow-y-auto p-4 space-y-2">
    ...
    <div class="flex justify-between items-center px-1 mb-1.5">
      <span class="text-[9px] text-[var(--text-muted)] font-bold uppercase tracking-wider">Danh sách file ({appState.files.length})</span>
    ...
    {#if appState.files.length === 0}
      ...
      >
        <FileUp size={24} class="mb-2 group-hover:scale-110 transition-transform" />
        <span class="text-xs font-bold text-[var(--text)]">Thêm file cấu hình</span>
        <span class="text-[9px] text-[var(--text-muted)] mt-1">Click chọn hoặc Kéo thả file Excel/CSV vào đây</span>
      </button>
    {:else}
      ...
      <div bind:this={listEl} class="space-y-2">
        {#each appState.files as file, idx (file.id)}
          ...
          <div 
            ...
            onclick={() => { if (!isDragging) appState.selectedFileIdx = idx; }}
            class="p-3 rounded-xl border text-left transition duration-200 relative group flex items-start gap-2
              {draggedIdx === idx && isDragging ? 'opacity-30 border-dashed border-[var(--border)] bg-transparent scale-95' : 'cursor-pointer'}
              {hoveredIdx === idx && isDragging && draggedIdx !== idx ? 'border-[var(--accent)] bg-[var(--accent-glow)]/30 scale-[1.02] shadow-lg shadow-[var(--accent)]/5' : ''}
              {appState.selectedFileIdx === idx && draggedIdx !== idx
                ? 'bg-[var(--active-file-bg)] border-[var(--accent)]/55 shadow-md shadow-[var(--accent)]/5' 
                : (draggedIdx !== idx ? 'bg-[var(--card-bg)] border-[var(--border)] hover:bg-[var(--active-file-bg)]/30 hover:border-zinc-500/30' : '')}"
          >
            <!-- Vach chi thi vi tri tha (Drop indicator line) -->
            {#if hoveredIdx === idx && isDragging && draggedIdx !== idx}
              {#if draggedIdx > idx}
                <div class="absolute -top-[5px] left-0 right-0 h-[3px] bg-[var(--accent)] rounded-full z-20 pointer-events-none"></div>
              {:else}
                <div class="absolute -bottom-[5px] left-0 right-0 h-[3px] bg-[var(--accent)] rounded-full z-20 pointer-events-none"></div>
              {/if}
            {/if}
            ...
            <!-- Kích hoạt build lại -->
            <div class="p-2 rounded-lg bg-zinc-500/5 shrink-0 transition-colors
              {ext === 'csv' 
            ...
            <div class="min-w-0 flex-1 space-y-1.5">
              <div class="text-xs font-bold truncate text-[var(--text)]" title={file.path}>
                {file.path.split(/[\\/]/).pop()}
              </div>
              ...
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <span class="px-1.5 py-0.5 rounded text-[8px] font-semibold bg-zinc-500/5 text-[var(--text-muted)] max-w-[80px] truncate">
            ...
            <div class="absolute right-2 top-2 hidden group-hover:flex items-center gap-0.5 bg-[var(--sidebar-bg)] border border-[var(--border)] rounded-md shadow-lg p-0.5 z-10">
              <button 
                onclick={(e) => { e.stopPropagation(); appState.moveFile(idx, 'up'); }}
                disabled={idx === 0}
              ...
              <button 
                onclick={(e) => { e.stopPropagation(); appState.moveFile(idx, 'down'); }}
              ...
              <button 
                onclick={(e) => { e.stopPropagation(); appState.removeFile(idx); }}
      ...
      <div class="pt-2">
        ...
        >
          <FileUp size={16} class="mb-1 group-hover:scale-110 transition-transform" />
          <span class="text-xs font-bold text-[var(--text)]">Thêm file cấu hình</span>
          <span class="text-[9px] text-[var(--text-muted)] mt-0.5">Click chọn hoặc Kéo thả file</span>
        </button>
      </div>
    {/if}
  </div>
    ...
    >
      <div class="flex items-start gap-2.5">
        ...
        <div class="flex-grow min-w-0 space-y-1">
          <div class="flex justify-between items-center text-[10px] font-bold tracking-wider uppercase text-[var(--accent)]">
            <span class="truncate pr-2">{status.main}</span>
            <span class="shrink-0">{Math.round(appState.progressPercent * 100)}%</span>
          </div>
          {#if status.sub}
            <div class="text-[9px] text-[var(--text-muted)] truncate" title={status.sub}>
              {status.sub}
            </div>
          {/if}
        </div>
